...
function isToday(time: Time): boolean {
  return (
    new Date().toLocaleDateString() === new Date(time).toLocaleDateString()
  );
}
...
function timeDifference(
  ...
  switch (unit) {
    ...
    default:
      ...
      const m = Math.floor(
        (ms - D * 1000 * 60 * 60 * 24 - h * 1000 * 60 * 60) / (1000 * 60)
      );
      // eslint-disable-next-line max-len
      const s = Math.floor(
        (ms - D * 1000 * 60 * 60 * 24 - h * 1000 * 60 * 60 - m * 1000 * 60) /
          1000
      );
      ms %= 1000;
      return (
